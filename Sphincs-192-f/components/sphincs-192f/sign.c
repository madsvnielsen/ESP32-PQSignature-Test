...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    uint8_t *seed = malloc(CRYPTO_SEEDBYTES);
    if (seed == NULL) {
        return -1;    }
...
int crypto_sign_signature(uint8_t *sig, size_t *siglen,
    ...
    uint8_t *optrand = (uint8_t *)malloc(SPX_N);
    uint8_t *mhash = (uint8_t *)malloc(SPX_FORS_MSG_BYTES);
    uint8_t *root = (uint8_t *)malloc(SPX_N);
    uint32_t i; 
    uint64_t tree;
    uint32_t idx_leaf;
    uint32_t *wots_addr = (uint32_t *)calloc(8, sizeof(uint32_t));
    uint32_t *tree_addr = (uint32_t *)calloc(8, sizeof(uint32_t));

    if (!optrand || !mhash || !root || !wots_addr || !tree_addr) {
        free(optrand);
        free(mhash);
        free(root);
        free(wots_addr);
...
int crypto_sign_verify(const uint8_t *sig, size_t siglen,
    ...
    uint8_t *mhash = (uint8_t *)malloc(SPX_FORS_MSG_BYTES);
    uint8_t *wots_pk = (uint8_t *)malloc(SPX_WOTS_BYTES);
    uint8_t *root = (uint8_t *)malloc(SPX_N);
    uint8_t *leaf = (uint8_t *)malloc(SPX_N);
    uint32_t *wots_addr = (uint32_t *)calloc(8, sizeof(uint32_t));
    uint32_t *tree_addr = (uint32_t *)calloc(8, sizeof(uint32_t));
    uint32_t *wots_pk_addr = (uint32_t *)calloc(8, sizeof(uint32_t));
    uint64_t tree;  
    uint32_t idx_leaf;
    unsigned int i;

    if (!mhash || !wots_pk || !root || !leaf || !wots_addr || !tree_addr || !wots_pk_addr) {
        free(mhash);
        free(wots_pk);
        free(root);
        free(leaf);
    ...
    int result = memcmp(root, pub_root, SPX_N) != 0 ? -1 : 0;
