#include <stdio.h>
#include <stdint.h>
#include "api.h" // MLDSA65 API header
...
{
    // KEY GENERATION
    uint8_t *public_key = (uint8_t *)malloc(PQCLEAN_MLDSA65_CLEAN_CRYPTO_PUBLICKEYBYTES);
    uint8_t *private_key = (uint8_t *)malloc(PQCLEAN_MLDSA65_CLEAN_CRYPTO_SECRETKEYBYTES);
    ...
    // Generate keypair
    if (PQCLEAN_MLDSA65_CLEAN_crypto_sign_keypair(public_key, private_key) != 0)
    {
        free(public_key);
        free(private_key);
        return;
    ...
    uint8_t *signature = malloc(PQCLEAN_MLDSA65_CLEAN_CRYPTO_BYTES);
    if (signature == NULL)
    {
