...
PQCLEAN_FALCON1024_CLEAN_crypto_sign_keypair(
    ...
    uint8_t *tmp_b = malloc(FALCON_KEYGEN_TEMP_10);
    int8_t *f = malloc(1024 * sizeof(int8_t));
    int8_t *g = malloc(1024 * sizeof(int8_t));
    int8_t *F = malloc(1024 * sizeof(int8_t));
    uint16_t *h = malloc(1024 * sizeof(uint16_t));
    ...
    PQCLEAN_FALCON1024_CLEAN_keygen(&rng, f, g, F, NULL, h, 10, tmp_b);
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    uint8_t *tmp_b = malloc(72 * 1024);
    int8_t *f = malloc(1024 * sizeof(int8_t));
    int8_t *g = malloc(1024 * sizeof(int8_t));
    int8_t *F = malloc(1024 * sizeof(int8_t));
    int8_t *G = malloc(1024 * sizeof(int8_t));
    int16_t *r_sig = malloc(1024 * sizeof(int16_t));
    uint16_t *r_hm = malloc(1024 * sizeof(uint16_t));

    unsigned char seed[48];
    inner_shake256_context sc;
    size_t u, v;

    if (!tmp_b || !f || !g || !F || !G || !r_sig || !r_hm) {
        free(tmp_b);
        free(f);
        free(g);
        free(F);
    ...
    if (!PQCLEAN_FALCON1024_CLEAN_complete_private(G, f, g, F, 10, tmp_b)) {
        free(tmp_b);
        free(f);
        free(g);
        free(F);
    ...
    PQCLEAN_FALCON1024_CLEAN_hash_to_point_ct(&sc, r_hm, 10, tmp_b);
    ...
    PQCLEAN_FALCON1024_CLEAN_sign_dyn(r_sig, &sc, f, g, F, G, r_hm, 10, tmp_b);
    v = PQCLEAN_FALCON1024_CLEAN_comp_encode(sigbuf, *sigbuflen, r_sig, 10);
...
do_verify(
    ...
    uint8_t *tmp_b = malloc(2 * 1024);
    uint16_t *h = malloc(1024 * sizeof(uint16_t));
    uint16_t *hm = malloc(1024 * sizeof(uint16_t));
    int16_t *sig = malloc(1024 * sizeof(int16_t));
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCON1024_CLEAN_hash_to_point_ct(&sc, hm, 10, tmp_b);
    inner_shake256_ctx_release(&sc);

    // Verify signature
    if (!PQCLEAN_FALCON1024_CLEAN_verify_raw(hm, sig, h, 10, tmp_b)) {
        free(tmp_b);
        free(h);
        free(hm);
        free(sig);
