...
PQCLEAN_FALCON512_CLEAN_crypto_sign_keypair(
    ...
    void *tmp_raw = malloc(FALCON_KEYGEN_TEMP_9 + sizeof(fpr));
    if (tmp_raw == NULL) {

        return -1;
    }

    // Align tmp to the alignment requirements of `fpr`
    uint8_t *tmp = (uint8_t *)(((uintptr_t)tmp_raw + sizeof(fpr) - 1) & ~(sizeof(fpr) - 1));
    ...
    PQCLEAN_FALCON512_CLEAN_keygen(&rng, f, g, F, NULL, h, 9, tmp);
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    uint8_t *tmp_b = malloc(72 * 512); // Equivalent to tmp.b
    if (tmp_b == NULL) {
        printf("Failed to allocate memory for tmp.b\n");
        return -1;
    }

    int8_t *f = malloc(512 * sizeof(int8_t));
    int8_t *g = malloc(512 * sizeof(int8_t));
    int8_t *F = malloc(512 * sizeof(int8_t));
    int8_t *G = malloc(512 * sizeof(int8_t));
    if (!f || !g || !F || !G) {
        printf("Failed to allocate memory for private key components\n");
        free(tmp_b);
        free(f);
        free(g);
        free(F);
    ...
    int16_t *r_sig = malloc(512 * sizeof(int16_t));
    uint16_t *r_hm = malloc(512 * sizeof(uint16_t));
    if (!r_sig || !r_hm) {
        printf("Failed to allocate memory for signature components\n");
        free(tmp_b);
        free(f);
        free(g);
        free(F);
    ...
    if (!PQCLEAN_FALCON512_CLEAN_complete_private(G, f, g, F, 9, tmp_b)) {
        goto failure;
    }
    ...
    PQCLEAN_FALCON512_CLEAN_hash_to_point_ct(&sc, r_hm, 9, tmp_b);
     ...
     */
    PQCLEAN_FALCON512_CLEAN_sign_dyn(r_sig, &sc, f, g, F, G, r_hm, 9, tmp_b);
    v = PQCLEAN_FALCON512_CLEAN_comp_encode(sigbuf, *sigbuflen, r_sig, 9);
